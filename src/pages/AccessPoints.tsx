...
const DraggableAPCard = React.memo(({
  ap,
  status,
  pinging,
  isSelected,
  ...
  ap: AccessPoint & { enabled?: boolean };
  status: any;
  pinging: any;
  isSelected: boolean;
...
const LocationGroup = ({
  ...
  id: string;
  title: string;
  aps: (AccessPoint & { enabled?: boolean })[];
  status: any;
  pinging: any;
  selectedIds: string[];
  onSelect: (c: AccessPoint, e: React.MouseEvent) => void;
  onPing: (c: AccessPoint) => void;
  switches: any[];
}) => {
...
const AccessPoints = () => {
  ...
  const [selectedDevice, setSelectedDevice] = useState<
    (AccessPoint & { enabled?: boolean }) | null
  >(null);
  ...
  const handleSaveAP = (
    ...
    const updatedAps = aps.map((a) =>
      a.id === id ? { ...a, name, ip, switchPort, switchId, enabled } : a,
    );
    setAps(updatedAps);
        ...
        {selectedDevice && (
          <aside className="w-[360px] border-l border-border bg-card shadow-sm h-full overflow-hidden z-30">
            <DeviceEditPanel
              ...
              onPing={(ip) =>
                handlePing({ ...selectedDevice, ip } as AccessPoint)
              }
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                value={newAP.ip}
                onChange={(e) => setNewAP({ ...newAP, ip: e.target.value })}
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setNewAP({ ...newAP, location: e.target.value })
                }
              >
                {LOCATIONS.map((l) => (
