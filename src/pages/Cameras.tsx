...
import { Server, Video, Activity, AlertCircle, CheckCircle2, MapPin, Trash2, Plus, Router, ShieldAlert } from 'lucide-react';
...
const DraggableCameraCard = ({ camera, status, pinging, isSelected, onClick, onPing, switches }: { camera: Camera & { enabled?: boolean }, status: any, pinging: any, isSelected: boolean, onClick: (e: React.MouseEvent) => void, onPing: (e: any) => void, switches: any[] }) => {
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center gap-2 mb-2">
                <Video className={`w-4 h-4 ${!isEnabled ? 'text-zinc-400' : (status === 'online' ? 'text-green-600' : status === 'offline' ? 'text-red-500' : 'text-muted-foreground')}`} />
                <span className="text-xs font-semibold text-muted-foreground">{camera.id}</span>
            </div>

            {/* Info */}
            <div className="space-y-0.5 mb-3">
                <h3 className="text-sm font-medium text-foreground truncate" title={camera.name}>{camera.name}</h3>
                <div className="flex items-center gap-1.5">
                    <p className="text-[10px] font-mono text-muted-foreground truncate">{camera.ip}</p>
                    {camera.switchPort && (
            ...
            >
                {pinging ? <Activity className="w-3 h-3 mr-1 animate-spin" /> :
                    !isEnabled ? <ShieldAlert className="w-3 h-3 mr-1" /> :
                        status === 'online' ? <CheckCircle2 className="w-3 h-3 mr-1" /> : <AlertCircle className="w-3 h-3 mr-1" />}
...
const LocationGroup = ({ id, title, cameras, status, pinging, selectedIds, onSelect, onPing, switches }: { id: string, title: string, cameras: (Camera & { enabled?: boolean })[], status: any, pinging: any, selectedIds: string[], onSelect: (c: Camera, e: React.MouseEvent) => void, onPing: (c: Camera) => void, switches: any[] }) => {
    ...
    return (
        <div ref={setNodeRef} className={`p-4 rounded-lg border transition-colors ${isOver ? 'bg-primary/5 border-primary/50' : 'bg-card/50 border-border/50'}`}>
            <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-muted-foreground ml-auto bg-muted px-2 py-0.5 rounded-full">{cameras.length}</span>
            </div>
...
const Cameras = () => {
    ...
    const [cameras, setCameras] = useState<(Camera & { enabled?: boolean })[]>(CAMERAS || []); // Safety
    const [pinging, setPinging] = useState<Record<string, boolean>>({});
    const [status, setStatus] = useState<Record<string, 'online' | 'offline' | null>>({});
    const [selectedDevice, setSelectedDevice] = useState<(Camera & { enabled?: boolean }) | null>(null);
    const [selectedIds, setSelectedIds] = useState<string[]>([]);
    const [activeId, setActiveId] = useState<string | null>(null);

    // Create State
    const [isCreating, setIsCreating] = useState(false);
    const [newCam, setNewCam] = useState({ name: '', ip: '', location: 'Térreo' });
    ...
    const handlePing = async (cam: Camera) => {
        ...
        try {
            ...
            setStatus(prev => ({ ...prev, [cam.id]: isAlive ? 'online' : 'offline' }));
    ...
    const handleSaveCamera = (id: string, name: string, ip: string, switchPort?: string, switchId?: string, enabled?: boolean) => {
        const updatedCams = cameras.map(c => c.id === id ? { ...c, name, ip, switchPort, switchId, enabled } : c);
        setCameras(updatedCams);
    ...
    const handleCreateCam = async () => {
        ...
        try {
            await fetch('/api/sync-cameras', { method: 'POST', body: JSON.stringify({ cameras: updated }), headers: { 'Content-Type': 'application/json' } });
            toast.success("Câmera Criada!");
        } catch (e) { toast.error("Erro ao salvar criação"); }
    };

    const camsByLocation = LOCATIONS.reduce((acc, loc) => {
                ...
                <div className="px-6 py-3 flex justify-between items-center">
                    <div className="flex items-center gap-6">
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-primary rounded-md"><Video className="w-5 h-5 text-primary-foreground" /></div>
                            <div>
                                <h1 className="text-base font-bold text-foreground">Câmeras</h1>
                                <p className="text-xs text-muted-foreground">{cameras.length} dispositivos</p>
                            </div>
                        </div>
                        ...
                        <div className="flex bg-muted/50 p-1 rounded-lg">
                            ...
                            <Button variant="secondary" size="sm"><Video className="w-4 h-4 mr-2" />Câmeras</Button>
                    ...
                    <div className="flex gap-4">
                        ...
                        <Button onClick={handlePingAll} variant="outline" size="sm"><Activity className="w-4 h-4 mr-2" />Verificar Todas</Button>
                    </div>
                </div>
            </header>
                    ...
                    <DragOverlay>
                        {activeId ? (
                            <div className="opacity-80 rotate-3 cursor-grabbing w-[200px]">
                                <Button variant="secondary" className="w-full justify-start"><Video className="w-4 h-4 mr-2" />Movendo Câmera...</Button>
                            </div>
                        ) : null}
                    </DragOverlay>
                </DndContext>
                ...
                <DialogContent>
                    ...
                    <div className="space-y-4 py-4">
                        <div className="space-y-2">
                            <Label>Nome</Label>
                            <Input value={newCam.name} onChange={e => setNewCam({ ...newCam, name: e.target.value })} placeholder="Ex: CAM - Hall" />
                        </div>
                        <div className="space-y-2">
                            <Label>IP</Label>
                            <Input value={newCam.ip} onChange={e => setNewCam({ ...newCam, ip: e.target.value })} placeholder="10.70.x.x" />
                        </div>
                        <div className="space-y-2">
