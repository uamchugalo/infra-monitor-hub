...
const saveReport = (report) => {
  const reports = fs.existsSync(REPORTS_FILE) ? JSON.parse(fs.readFileSync(REPORTS_FILE)) : [];
  reports.unshift(report);
  // Increase limit to 500 to keep more history (auto + manual)
  fs.writeFileSync(REPORTS_FILE, JSON.stringify(reports.slice(0, 500), null, 2));
};
...
const runMonitoringCheck = async (isAutomated = true) => {
  console.log(`Starting ${isAutomated ? 'Automated' : 'Manual'} Monitoring Check...`);
  ...
  const checkAndUpdate = async (dev, type, table) => {
    ...
    try {
      ...
      if (res.alive) {
        ...
        report.details.push({ name: dev.name, ip: dev.ip, type, status: 'SUCCESS' });
      } else {
        report.failures.push({ name: dev.name, ip: dev.ip, type, reason: 'No Ping Response' });
        report.details.push({ name: dev.name, ip: dev.ip, type, status: 'FAILED' });
      }

      // Update Database & History
      if (table === 'camera') {
        await prisma.camera.update({ where: { id: dev.id }, data: { status } }).catch(() => { });
        if (isAutomated) {
          await prisma.cameraLog.create({ data: { cameraId: dev.id, status } }).catch(() => { });
        }
      } else if (table === 'accessPoint') {
        await prisma.accessPoint.update({ where: { id: dev.id }, data: { status } }).catch(() => { });
        if (isAutomated) {
          await prisma.accessPointLog.create({ data: { apId: dev.id, status } }).catch(() => { });
        }
      }
  ...
  const camPromises = enabledCameras.map(c => checkAndUpdate(c, 'CAMERA', 'camera'));
  const apPromises = enabledAps.map(a => checkAndUpdate(a, 'AP', 'accessPoint'));

  await Promise.all([...camPromises, ...apPromises]);
...
cron.schedule('0 0 * * *', async () => {
  ...
  await prisma.cameraLog.deleteMany({ where: { timestamp: { lt: tenDaysAgo } } });
  await prisma.accessPointLog.deleteMany({ where: { timestamp: { lt: tenDaysAgo } } });
...
app.post('/api/sync-cameras', async (req, res) => {
  ...
  if (!Array.isArray(cameras)) return res.status(400).json({ error: 'Invalid data' });
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const c of cameras) {
      await tx.camera.upsert({
        where: { id: c.id },
        create: { ...c, enabled: c.enabled ?? true, switchId: c.switchId || null, switchPort: c.switchPort || null },
        update: { ...c, enabled: c.enabled ?? true, switchId: c.switchId || null, switchPort: c.switchPort || null }
      });
      ...
      if (c.switchId && c.switchPort) {
        ...
        await tx.switchPort.upsert({
          ...
          update: { deviceId: c.id, deviceIp: c.ip, name: c.name, type: 'Camera' },
          create: { switchId: c.switchId, port: portNum, deviceId: c.id, deviceIp: c.ip, name: c.name, type: 'Camera' }
        }).catch(async () => {
          // Fallback if upsert fails due to missing unique constraint key in some prisma setups
          const existing = await tx.switchPort.findFirst({ where: { switchId: c.switchId, port: portNum } });
          if (existing) await tx.switchPort.update({ where: { id: existing.id }, data: { deviceId: c.id, deviceIp: c.ip, name: c.name, type: 'Camera' } });
          else await tx.switchPort.create({ data: { switchId: c.switchId, port: portNum, deviceId: c.id, deviceIp: c.ip, name: c.name, type: 'Camera' } });
        });
      }
    }
  });
...
app.post('/api/sync-aps', async (req, res) => {
  const { aps } = req.body;
  if (!Array.isArray(aps)) return res.status(400).json({ error: 'Invalid data' });
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const a of aps) {
      await tx.accessPoint.upsert({
        where: { id: a.id },
        create: { ...a, enabled: a.enabled ?? true, switchId: a.switchId || null, switchPort: a.switchPort || null },
        update: { ...a, enabled: a.enabled ?? true, switchId: a.switchId || null, switchPort: a.switchPort || null }
      });
      ...
      if (a.switchId && a.switchPort) {
        ...
        await tx.switchPort.upsert({
          ...
          update: { deviceId: a.id, deviceIp: a.ip, name: a.name, type: 'AP' },
          create: { switchId: a.switchId, port: portNum, deviceId: a.id, deviceIp: a.ip, name: a.name, type: 'AP' }
        }).catch(async () => {
          const existing = await tx.switchPort.findFirst({ where: { switchId: a.switchId, port: portNum } });
          if (existing) await tx.switchPort.update({ where: { id: existing.id }, data: { deviceId: a.id, deviceIp: a.ip, name: a.name, type: 'AP' } });
          else await tx.switchPort.create({ data: { switchId: a.switchId, port: portNum, deviceId: a.id, deviceIp: a.ip, name: a.name, type: 'AP' } });
        });
      }
    }
  });
...
app.post('/api/sync-pcs', async (req, res) => {
  const { pcs } = req.body;
  if (!Array.isArray(pcs)) return res.status(400).json({ error: 'Invalid data' });
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const p of pcs) {
        ...
        if (p.history && Array.isArray(p.history)) {
          ...
          await tx.pCLog.deleteMany({ where: { pcId: pcId, id: { notIn: logIds } } });
          for (const entry of p.history) {
            await tx.pCLog.upsert({
              where: { id: entry.id },
              create: { id: entry.id, pcId: pcId, timestamp: entry.timestamp, message: entry.message },
...
app.post('/api/sync-switches', async (req, res) => {
  ...
  if (!Array.isArray(switches)) return res.status(400).json({ error: 'Invalid data' });
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const s of switches) {
        await tx.switch.upsert({
          where: { id: s.id },
          update: { name: s.name, ip: s.ip, model: s.model || '', location: s.location, totalPorts: s.totalPorts, order: s.order, enabled: s.enabled ?? true },
          create: { id: s.id, name: s.name, ip: s.ip, model: s.model || '', location: s.location, totalPorts: s.totalPorts, order: s.order, enabled: s.enabled ?? true }
        });
        ...
        if (s.ports) {
          ...
          await tx.switchPort.deleteMany({ where: { switchId: s.id, port: { notIn: portNums } } });

          for (const p of s.ports) {
            const portData = { switchId: s.id, port: p.port, name: p.name, type: p.type, deviceId: p.deviceId, deviceIp: p.deviceIp };
            const existing = await tx.switchPort.findFirst({ where: { switchId: s.id, port: p.port } });
            if (existing) {
              await tx.switchPort.update({ where: { id: existing.id }, data: portData });
            } else {
...
app.post('/api/sync-patch-panels', async (req, res) => {
  ...
  if (!Array.isArray(patchPanels)) return res.status(400).json({ error: 'Invalid data' });
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const p of patchPanels) {
        await tx.patchPanel.upsert({
          where: { id: p.id },
          update: { name: p.name, location: p.location, totalPorts: p.totalPorts, order: p.order },
          create: { id: p.id, name: p.name, location: p.location, totalPorts: p.totalPorts, order: p.order }
        });

        if (p.ports) {
          const portNums = p.ports.map(pt => pt.port);
          await tx.patchPanelPort.deleteMany({ where: { patchPanelId: p.id, port: { notIn: portNums } } });

          for (const pt of p.ports) {
            const ptData = { patchPanelId: p.id, port: pt.port, description: pt.description };
            const existing = await tx.patchPanelPort.findFirst({ where: { patchPanelId: p.id, port: pt.port } });
            if (existing) await tx.patchPanelPort.update({ where: { id: existing.id }, data: ptData });
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const log of logs) {
        await tx.labLog.upsert({
          where: { id: log.id },
          update: { timestamp: log.timestamp, message: log.message, labId: log.labId || '' },
          create: { id: log.id, timestamp: log.timestamp, message: log.message, labId: log.labId || '' }
        });
      }
    });
    res.json({ success: true });
...
app.get('/api/last-status', (req, res) => {
  try {
    ...
    const report = fs.existsSync(LATEST_STATUS_FILE) ? JSON.parse(fs.readFileSync(LATEST_STATUS_FILE)) : null;
      ...
      if (reports.length > 0) {
        ...
        if (last && last.details) {
          last.details.forEach(d => {
            if (d.ip) statusMap[d.ip] = d.status === 'SUCCESS' ? 'online' : 'offline';
          });
        }
...
app.post('/api/ping', async (req, res) => {
  ...
  exec(command, (error, stdout, stderr) => {
    ...
    const isAlive = !error && (output.toLowerCase().includes('ttl=') || output.toLowerCase().includes('time='));
    let time = 0;
