    ...
    for (const lab of labs) {
        ...
        for (const pc of lab.data) {
            ...
            try {
                ...
                // Normaliza MAC
                const cleanMac = pc.mac ? pc.mac.replace(/-/g, ':').toUpperCase() : `00:00:00:00:00:${index}`;
                ...
                if (e.code === 'P2002') {
                    console.log(`Conflito de MAC (${pc.mac}). Atualizando registro existente...`);
                    const existing = await prisma.pC.findUnique({ where: { mac: pc.mac.toUpperCase() } });
                    if (existing) {
